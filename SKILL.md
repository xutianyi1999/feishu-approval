...
装入 OpenClaw 时建议包含：**`SKILL.md`**、**`embedded-docs/`**、**`docs/`**（**`docs/AI.md`**、**`docs/examples/`**、**`docs/approval-code-map.local.template.md`**）、本机 **`approval-code-map.local.md`**（**勿**提交；可由 **`util init`** 生成）。映射规则见 **`docs/AI.md` §8**。可选 **`AGENTS.md`**（仅仓库开发）。目录名建议 `feishu-approval`。
