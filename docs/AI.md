...
| `util validate-widgets` | 离线：校验数组及 **`fieldList` / `fieldListMobile` 内层**；**`image` / `attachment` / `attachmentV2`** 的 **`value` 须为数组**（勿 `null`，空 **`[]`**）。**`date` / `amount` / `formula`** 启发式与 **`--fix`** 见 **`--help`** 与 **§7** |
...
| `instance` | get / list / **create**（**`--widgets-json-file`** 或 **`--form` / `--form-file`** 或 **`--wizard`** 或 **`--template expense`**；可选 **`--validate-against-json`**、**`--dry-run`**）/ query / … |
...
**`instance create`** 与 **`util validate-widgets`** 对 **`form` 解析后的数组**做同源离线检查（**`fieldList` 内层**须 **`id`+`type`+`value`** 等）。**`--validate-against-json`** 不覆盖 **value** 业务规则。飞书 **「index=0」** 类文案：对照 stderr 路径（如 **`widgets[2].value[0][1]`**），勿默认第一个根控件有错。
...
| **`formula` 空** | API 常要求非空 |
| **`code` ≠ 0**（附件数组、`attachmentV2` 等） | stderr **`[feishu-approval-tool]`**；**`util explain --msg`**；**`util validate-widgets`**（**`--fix`**） |
